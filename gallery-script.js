...
function fitBoardToScreen(config, aspectRatio) {
    ...
    // Доступное пространство (без отступов) - учитываем padding багета (25px с каждой стороны)
    const framePadding = 50; // 25px слева + 25px справа
    const availableWidth = containerRect.width - framePadding;
    const availableHeight = containerRect.height - framePadding;
    ...
    // Применяем размеры к доске пазла
    ...
    // Багет автоматически подстроится под размеры доски
    
    // Обновляем CSS переменную для соотношения сторон
